...
class ApiGetMembersIntegrationTest extends TestCase {
    ...
    public function test_query_members_by_house(): void {
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? LIMIT 1', 1);
    ...
    public function test_query_members_by_party(): void {
        $house = 1;
        $party = 'ALP';
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND party LIKE ?', $house, "%$party%");
        $this->assertIsObject($q);
    }
    ...
    public function test_query_members_by_constituency(): void {
        $house = 1;
        $state = 'NSW';
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND constituency LIKE ?', $house, "%$state%");
        $this->assertIsObject($q);
    }
    ...
    public function test_query_members_with_date_range(): void {
        $house = 1;
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND entered_house <= NOW() AND NOW() <= left_house', $house);
        $this->assertIsObject($q);
    }
    ...
    public function test_query_members_search_first_name(): void {
        $house = 1;
        $search = '%Smith%';
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND first_name LIKE ?', $house, $search);
        $this->assertIsObject($q);
    }
    ...
    public function test_query_members_search_last_name(): void {
        $house = 1;
        $search = '%John%';
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND last_name LIKE ?', $house, $search);
        $this->assertIsObject($q);
    }
    ...
    public function test_query_members_search_full_name(): void {
        $house = 1;
        $search = '%John Smith%';
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND CONCAT(first_name, \' \', last_name) LIKE ?', $house, $search);
        $this->assertIsObject($q);
    }
    ...
    public function test_member_table_structure(): void {
        $q = getParlDB()->query('DESCRIBE member');
        $this->assertIsObject($q);
    ...
    public function test_member_query_rows_method(): void {
        $q = getParlDB()->query('SELECT * FROM member LIMIT 1');
    ...
    public function test_senate_search_includes_constituency(): void {
        $house = 2;
        $search = '%NSW%';
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND constituency LIKE ?', $house, $search);
        $this->assertIsObject($q);
    }
    ...
    public function test_state_search_house(): void {
        $house = 1;
        $state = 'NSW';
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND constituency LIKE ? AND entered_house <= NOW() AND NOW() <= left_house', $house, "%$state%");
        $this->assertIsObject($q);
    }
    ...
    public function test_multiple_state_searches(): void {
        ...
        foreach ($states as $state) {
            $q = getParlDB()->query('SELECT person_id FROM member WHERE constituency LIKE ? LIMIT 1', "%$state%");
    ...
    public function test_search_house_no_constituency(): void {
        ...
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND (first_name LIKE ? OR last_name LIKE ? OR CONCAT(first_name, \' \', last_name) LIKE ?)', $house, '%test%', '%test%', '%test%');
        $this->assertIsObject($q);
    }
    ...
    public function test_search_senate_with_constituency(): void {
        ...
        $q = getParlDB()->query('SELECT * FROM member WHERE house = ? AND (first_name LIKE ? OR last_name LIKE ? OR CONCAT(first_name, \' \', last_name) LIKE ? OR constituency LIKE ?)', $house, '%test%', '%test%', '%test%', '%test%');
        $this->assertIsObject($q);
    }
