...
class SEARCHENGINE {
        ...
        foreach ($all_words as $word) {
            ...
            if (strpos($word, ':') !== FALSE) {
                ...
                if ($type == "groupby") {
                    ...
                    if ($value == "debates" || $value == "debate" || $value == "department" || $value == "departments" || $value == "dept") {
                        $value = "debate";
                    }
    ...
    public function highlight($body) {
        ...
        // XXX Horrible hack.
        $hlbody = preg_replace('#<(/?)<span class="hi">a</span>([^>]*?)>#', "<\\1a\\2>", $hlbody);
        // Collapse duplicates.
    ...
    public function position_of_first_word($body) {
        // Spaces to make regexp mapping easier.
        $lcbody = ' ' . strtolower($body) . ' ';
        $pos = -1;
...
function search_by_usage($search, $house = 0) {
    ...
    for ($n = 0; $n < $q->rows(); $n++) {
        ...
        if ($hdate < $mindate[$speaker_id]) {
            $mindate[$speaker_id] = $hdate;
        }
        if ($hdate > $maxdate[$speaker_id]) {
            $maxdate[$speaker_id] = $hdate;
        }
    }

    // Fetch details of all the speakers.
    if (count($speaker_count)) {
        // TODO - validate $house is an acceptable value.
    ...
    foreach ($speaker_count as $speaker_id => $count) {
        ...
        if ($mindate[$speaker_id] < $speakers[$pid]['pmindate']) {
            $speakers[$pid]['pmindate'] = $mindate[$speaker_id];
        }
        if ($maxdate[$speaker_id] > $speakers[$pid]['pmaxdate']) {
            $speakers[$pid]['pmaxdate'] = $maxdate[$speaker_id];
        }
    }
