...
class MEMBER {
	...
	function MEMBER ($args) {
		...
		$q = $this->db->query("SELECT member_id, house, title,
			...
			FROM member
			WHERE person_id = '" . mysqli_real_escape_string($db, $person_id) . "'
	...
	function member_id_to_person_id ($member_id) {
		...
		$q = $this->db->query("SELECT person_id FROM member 
					WHERE member_id = '" . mysqli_real_escape_string($db, $member_id) . "'");
		if ($q->rows > 0) {
	...
	function constituency_to_person_id ($constituency) {
		...
		if ($q->rows > 0) {
			...
			$q = $this->db->query("SELECT person_id FROM member WHERE constituency = '".mysqli_real_escape_string($db, $constituency)."' ORDER BY left_house DESC LIMIT 1");
			if ($q->rows > 0) {
				return $q->field(0, 'person_id');
			} else {
	...
	function name_to_person_id ($name, $const='') {
		...
		#		return false;
		#	}
		#	$title = mysqli_real_escape_string($db, $m[1]);
		#	$last_name = mysqli_real_escape_string($db, $m[2]);
		#	$const = $m[3];
		...
		if ($this_page=='msp') {
			...
			$first_name = mysqli_real_escape_string($db, $m[1]);
			$middle_name = mysqli_real_escape_string($db, $m[2]);
			$last_name = mysqli_real_escape_string($db, $m[3]);
			$q .= "house = 4 AND (";
			...
			if (strstr($this_page, 'mp'))
				...
				$house = 2;
			# if ($title) $q .= 'title = \'' . mysqli_real_escape_string($db, $title) . '\' AND ';
			$q .= "house = ".$house." AND ((first_name='".mysqli_real_escape_string($db, $first_name." ".$middle_name)."' AND last_name='".mysqli_real_escape_string($db, $last_name)."') OR ".
			"(first_name='".mysqli_real_escape_string($db, $first_name)."' AND last_name='".mysqli_real_escape_string($db, $middle_name." ".$last_name)."'))";
			if ($const) {
		...
		if ($const) {
			$q .= ' AND constituency=\''.mysqli_real_escape_string($db, $const)."'";
		}
	...
	$q = $this->db->query('SELECT * FROM moffice WHERE person=' .
		mysqli_real_escape_string($db, $this->person_id) . ' ORDER BY from_date DESC');
	for ($row=0; $row<$q->rows(); $row++) {
	...
	#               FROM 	memberinfo
	#               WHERE	member_id = '" . mysqli_real_escape_string($db, $this->member_id) . "'
	#               ");
        $q = $this->db->query("SELECT data_key, data_value
                        FROM 	memberinfo
                        WHERE	member_id = '" . mysqli_real_escape_string($db, $this->member_id) . "'
	...
	#               FROM 	personinfo
	#               WHERE	person_id = '" . mysqli_real_escape_string($db, $this->person_id) . "'
	#               ");
        $q = $this->db->query("SELECT data_key, data_value
                        FROM 	personinfo
                        WHERE	person_id = '" . mysqli_real_escape_string($db, $this->person_id) . "'
        ...
        $q = $this->db->query("SELECT	data_key,
                                data_value
                        FROM 	consinfo
                        WHERE	constituency = '" . mysqli_real_escape_string($db, $this->constituency) . "'
                        ");
        for ($row = 0; $row < $q->rows(); $row++)
        {
            $this->extra_info[$q->field($row, 'data_key')] = $q->field($row, 'data_value');
