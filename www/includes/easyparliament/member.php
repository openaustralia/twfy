...
class MEMBER {
	...
	var $reasons = array(
		...
		'general_election_standing' 	=> array('Federal election (standing again)', 'Federal election (stood again)'),
	...
	function MEMBER ($args) {
		...
		$q = $this->db->query("SELECT member_id, house, title,
			...
			FROM member
			WHERE person_id = '" . mysqli_real_escape_string($person_id) . "'
	...
	function member_id_to_person_id ($member_id) {
		...
		$q = $this->db->query("SELECT person_id FROM member 
					WHERE member_id = '" . mysqli_real_escape_string($member_id) . "'");
		if ($q->rows > 0) {
	...
	function constituency_to_person_id ($constituency) {
		...
		if ($normalised) $constituency = $normalised;
	        ...
	        $q = $this->db->query("SELECT person_id FROM member 
					WHERE constituency = '" . mysqli_real_escape_string($constituency) . "' 
					AND left_reason = 'still_in_office'");

		if ($q->rows > 0) {
			return $q->field(0, 'person_id');
		} else {
			$q = $this->db->query("SELECT person_id FROM member WHERE constituency = '".mysqli_real_escape_string($constituency)."' ORDER BY left_house DESC LIMIT 1");
			if ($q->rows > 0) {
				return $q->field(0, 'person_id');
			} else {
	...
	function name_to_person_id ($name, $const='') {
		...
		#		return false;
		#	}
		#	$title = mysqli_real_escape_string($m[1]);
		#	$last_name = mysqli_real_escape_string($m[2]);
		#	$const = $m[3];
		...
		if ($this_page=='msp') {
			...
			$first_name = mysqli_real_escape_string($m[1]);
			$middle_name = mysqli_real_escape_string($m[2]);
			$last_name = mysqli_real_escape_string($m[3]);
			$q .= "house = 4 AND (";
			...
			if (strstr($this_page, 'mp'))
				...
				$house = 2;
			# if ($title) $q .= 'title = \'' . mysqli_real_escape_string($title) . '\' AND ';
			$q .= "house = ".$house." AND ((first_name='".mysqli_real_escape_string($first_name." ".$middle_name)."' AND last_name='".mysqli_real_escape_string($last_name)."') OR ".
			"(first_name='".mysqli_real_escape_string($first_name)."' AND last_name='".mysqli_real_escape_string($middle_name." ".$last_name)."'))";
			if ($const) {
		...
		if ($const) {
			$q .= ' AND constituency=\''.mysqli_real_escape_string($const)."'";
		}
	...
	$q = $this->db->query('SELECT * FROM moffice WHERE person=' .
		mysqli_real_escape_string($this->person_id) . ' ORDER BY from_date DESC');
	for ($row=0; $row<$q->rows(); $row++) {
	...
	#               FROM 	memberinfo
	#               WHERE	member_id = '" . mysqli_real_escape_string($this->member_id) . "'
	#               ");
        $q = $this->db->query("SELECT data_key, data_value
                        FROM 	memberinfo
                        WHERE	member_id = '" . mysqli_real_escape_string($this->member_id) . "'
	...
	#               FROM 	personinfo
	#               WHERE	person_id = '" . mysqli_real_escape_string($this->person_id) . "'
	#               ");
        $q = $this->db->query("SELECT data_key, data_value
                        FROM 	personinfo
                        WHERE	person_id = '" . mysqli_real_escape_string($this->person_id) . "'
        ...
        $q = $this->db->query("SELECT	data_key,
                                data_value
                        FROM 	consinfo
                        WHERE	constituency = '" . mysqli_real_escape_string($this->constituency) . "'
                        ");
        for ($row = 0; $row < $q->rows(); $row++)
        {
            $this->extra_info[$q->field($row, 'data_key')] = $q->field($row, 'data_value');
