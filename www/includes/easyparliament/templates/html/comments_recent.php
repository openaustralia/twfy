...
if (isset($data['comments']) && count($data['comments']) > 0) {
    ...
    <ul>
        ...
        foreach ($data['comments'] as $n => $comment) {
            ...
            ?><a name="c<?php echo $count; ?>"></a><strong><?php echo htmlentities($comment['firstname'] . ' ' . $comment['lastname']); ?>:</strong>
                <?php echo $commenttext; ?> <small>(<?php echo relative_time($comment['posted']); ?>)</small><br><a
                    href="<?php echo $comment['url']; ?>">Read comment</a> | <a href="<?php echo $USERURL->generate(); ?>"
    ...
    if ($this_page == 'home') {
        ...
        ?>
        <p><a href="<?php echo $MOREURL->generate(); ?>#c<?php echo count($data['comments']) + 1; ?>">See more comments posted
                recently</a></p>
        <?php
    }
    if ($this_page != 'home')
