...
if (isset($data['comments']) && count($data['comments']) > 0) {
    ...
    foreach ($data['comments'] as $n => $comment) {
        ...
        <li>
            <p><a
                    name="c<?php echo $count; ?>"></a><strong><?php echo htmlentities($comment['firstname'] . ' ' . $comment['lastname']); ?>:</strong>
                <?php echo $commenttext; ?> <small>(<?php echo relative_time($comment['posted']); ?>)</small> <a
                    href="<?php echo $comment['url']; ?>">Read comment</a></p>
        </li>
        <?php
    }
    print '</ul>';
