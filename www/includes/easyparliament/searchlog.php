...
class SEARCHLOG {
	...
	function add ($searchlogdata) {
        ...
        $this->db->query("INSERT INTO search_query_log
            ...
            VALUES ('" . mysqli_real_escape_string($searchlogdata['query']) . "',
	...
	$htmlescape = 1;
	if ($pos = strpos($query, ':')) {
		$qq = $this->db->query('SELECT first_name, last_name FROM member WHERE person_id="'.mysqli_real_escape_string(substr($query, $pos+1)).'" LIMIT 1');
		if ($qq->rows()) {
