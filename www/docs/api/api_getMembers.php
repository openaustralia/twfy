...
function api_getMembers_party($house, $s) {
	...
	_api_getMembers_output('select * from member
		where house = ' . mysqli_real_escape_string($house) . '
		and party like "%' . mysqli_real_escape_string($s) .
...
function api_getMembers_state($house, $s) {
        ...
        _api_getMembers_output('select * from member
                where house = ' . mysqli_real_escape_string($house) . '
                and constituency like "%' . mysqli_real_escape_string($s) .
                '%" and entered_house <= date(now()) and date(now()) <= left_house');
}

function api_getMembers_search($house, $s) {
	$sq = mysqli_real_escape_string($s);
	_api_getMembers_output('select * from member
		where house = ' . mysqli_real_escape_string($house) . "
...
function api_getMembers($house, $date = 'now()') {
	_api_getMembers_output('select * from member where house=' . mysqli_real_escape_string($house) .
