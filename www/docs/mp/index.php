...
$cconstituency = strtolower(str_replace(['_', '.', ' and '], [' ', '&amp;', ' &amp; '], get_http_var('c')));
...
if ($name == 'sian james') {
    $name = "si&acirc;n james";
}
if ($name == 'lembit opik') {
    $name = "lembit &ouml;pik";
}
if ($name == 'bairbre de brun') {
    $name = "bairbre de br&uacute;n";
}
if ($name == 'daithi mckay') {
    $name = 'daith&iacute; mckay';
}
if ($name == 'caral ni chuilin') {
    $name = 'car&aacute;l n&iacute; chuil&iacute;n';
}
if ($name == 'a j beith') {
    $name = 'alan beith';
    $redirect = TRUE;
    ...
    if (validate_postcode($pc)) {
        ...
        if ($constituency == "connection_timed_out") {
            ...
            foreach ($constituency as $c) {
                $member = new MEMBER(['constituency' => $c]);
            ...
            if ($MEMBER->person_id()) {
                if ($THEUSER->isloggedin()) {
                    ...
                    $details = [
                        ...
                        'email' => $THEUSER->email(),
                        'emailpublic' => $THEUSER->emailpublic(),
                        'postcode' => $pc,
    ...
    if (array_key_exists('office', $MEMBER->extra_info())) {
        ...
        foreach ($office as $row) {
            if ($row['to_date'] != '9999-12-31') {
                ...
                $mins .= ' (';
                if (
                    !($row['source'] == 'chgpages/selctee' && $row['from_date'] == '2004-05-28')
