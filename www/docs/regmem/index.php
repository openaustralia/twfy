  ...
  foreach ($files as $_) {
    ...
    if (preg_match('#<regmem personid="uk.org.publicwhip/person/' . $p . '" memberid="(.*?)" membername="(.*?)" date="(.*?)">(.*?)</regmem>#s', $file, $m)) {
      ...
      if (!$name) {
        ...
        print $link;
        ?>
<p>This page shows how <a href="/mp/?p=<?php echo $p ?>"><?php echo $name ?></a>'s entry in the Register of Members' Interests has changed over time, starting at the most recent and working back to the earliest we have managed to parse.</p>
<table id="regmem">
...
function register_history($f) {
  ...
  ?>
<p>This page shows all the changes in the Register of Members' Interests between the editions of <a href="./?d=<?php echo $old_iso ?>"><?php echo $old_pretty ?></a> and <a href="./?d=<?php echo $new_iso ?>"><?php echo $new_pretty ?></a>, in alphabetical order by MP.</p>
...
function show_register($d) {
  ...
  print $link;
  ?>
<p>This page shows the Register of Members' Interests as released on <?php echo $d_pretty ?>, in alphabetical order by MP.
  <?php if ($d_iso > '2002-05-14') {
    ?><a href="./?f=<?php echo $d_iso ?>">Compare this edition with the one before it</a></p><?php
  } ?>
<div id="regmem">
  <?php
  uksort($data, 'by_name_ref');
