  ...
  foreach ($files as $_) {
    ...
    if (preg_match('#<regmem personid="uk.org.publicwhip/person/' . $p . '" memberid="(.*?)" membername="(.*?)" date="(.*?)">(.*?)</regmem>#s', $file, $m)) {
      ...
      if (!$name) {
        ...
        print $link;
        ?>
                <p>This page shows how <a href="/mp/?p=<?php echo $p ?>"><?php echo $name ?></a>'s entry in the Register of Members'
                    Interests has changed over time, starting at the most recent and working back to the earliest we have managed to
                    parse.</p>
                <table id="regmem">
                    <tr>
                        <th width="50%">Removed</th>
                        <th width="50%">Added</th>
                    </tr>
                    <?php
      }
      $name = $m[2];
...
function register_history($f) {
  ...
  ?>
        <p>This page shows all the changes in the Register of Members' Interests between the editions of <a
                href="./?d=<?php echo $old_iso ?>"><?php echo $old_pretty ?></a> and <a
                href="./?d=<?php echo $new_iso ?>"><?php echo $new_pretty ?></a>, in alphabetical order by MP.</p>
...
function front_page() {
  ...
  ?>
            <p>This section of the site lets you see how MPs' entries in the Register of Members' Interests have changed
                over time, either by MP, or for a particular issue of the Register.</p>

            <blockquote>
                <p style="margin-top:0">The financial thresholds over which an interest must be registered are mainly based,
                    for convenience, on percentages of an MP's salary: one per cent, or currently �590, for employment,
                    gifts and hospitality; ten per cent, or �5,900, for rental income; and a hundred per cent, or �59,000,
                    for property and shares. The exception is sponsorship, where the threshold has been set at �1,000 to
                    match that set for registration with the Electoral Commission.</p>
                <p>Continuing interests like employment or property remain on the Register until the Member asks for them to
                    be removed. 'One-off' benefits like gifts, visits and donations appear with their date of registration
                    and remain on the Register for a year from that date and until they have appeared in one printed
                    Register.</p>
                <p style="margin-bottom:0" align="right">&mdash; <a
                        href="http://www.publications.parliament.uk/pa/cm/cmregmem/051214/memi01.htm">http://www.publications.parliament.uk/pa/cm/cmregmem/051214/memi01.htm</a>
                    (with more information).</p>
            </blockquote>
            <p>So, either <strong>pick an issue to compare against the one previous:</strong></p>
            <p align="center"><?php echo $compare ?></p>
            <p><strong>View a particular edition of the Register of Members' Interests:</strong></p>
            <p align="center"><?php echo $view ?></p>
            <p>Or <strong>view the history of an MP's entry in the Register:</strong></p>
            <ul id="mps">
                <?php
...
function show_register($d) {
  ...
  print $link;
  ?>
                <p>This page shows the Register of Members' Interests as released on <?php echo $d_pretty ?>, in
                    alphabetical order by MP.
                    <?php if ($d_iso > '2002-05-14') {
                      ?><a href="./?f=<?php echo $d_iso ?>">Compare this edition with the one before it</a>
                    </p><?php
                    } ?>
                <div id="regmem">
                    <?php
                    ...
                    foreach ($data as $person_id => $v) {
                      ...
                      if ($out) {
                        $PAGE->block_start(['title' => '<a name="' . $person_id . '"></a>' . $names[$person_id] . ' - <a href="?p=' . $person_id . '">Register history</a> | <a href="http://www.openaustralia.org/mp/?pid=' . $person_id . '">MP\'s page</a>']);
                        print "\n$out";
                        $PAGE->block_end();
                      }
                    }
