...
function your_mp_bullet_point() {
  ...
  if ($THEUSER->constituency_is_set()) {
    ...
    if ($MEMBER->valid) {
      ...
      ?>
    <p><a href="<?php echo $MPURL->generate(); ?>"><strong>Find out more about <?php echo $mpname; ?>, your <?php echo $former ?> Representative</strong></a>
...
?>
                        ...
                        <p><strong><label for="s">Search<?php echo get_http_var("keyword") ? ' Hansard for \'' . htmlspecialchars(get_http_var("keyword")) . '\'' : ''?>:</label></strong>
                    <input type="text" name="s" id="s" size="15" maxlength="100" class="text" value="<?php echo htmlspecialchars(get_http_var("keyword"))?>">&nbsp;&nbsp;<input type="submit" value="SEARCH" class="submit"></p>
                        <?php
...
function email_alert_bullet_point() {
  if (get_http_var("keyword")) { ?>
        <li><p><a href="<?php echo WEBPATH . "alert?keyword=" . htmlspecialchars(get_http_var('keyword'))?>&only=1"><strong>Sign up to be emailed when '<?php echo htmlspecialchars(get_http_var('keyword'))?>' is mentioned in Parliament</strong></a></p></li>
  <?php }
  else { ?>
        <li><p><a href="<?php echo WEBPATH . "alert/"?>"><strong>Sign up to be emailed when something relevant to you happens in Parliament</strong></a></p></li>
  <?php }
}
